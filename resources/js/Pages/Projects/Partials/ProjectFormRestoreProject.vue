...
function submit() {
    form.post(route('projects.restore', props.project.id), {
        onSuccess: () => {
            form.reset();
            toggleModal(false);
        }
...
<template>
    ...
    <Modal :show="modalOpen" @close="toggleModal">
        ...
        <form class="project_form_create_project" @submit.prevent="submit">
            ...
            <h3 class="title">Restore project</h3>

            <p class="text-sm text-gray-600 py-2">This project has been deleted, but you can restore it. </p>
